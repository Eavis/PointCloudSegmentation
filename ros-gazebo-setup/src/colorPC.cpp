              ...
              const CameraInfoConstPtr& cinfo_msg)
{
    ...
    for(pcl::PointCloud<pcl::PointXYZ>::iterator pt = cloud->points.begin(); pt < cloud->points.end(); pt++)
    {
        ...
        if(uv.x>0 && uv.x < image.cols && uv.y > 0 && uv.y < image.rows)
        {
            ...
            p.y = (*pt).y;
            p.z = (*pt).z;
            p.r = image.at<cv::Vec3b>(uv)[0];
            p.g = image.at<cv::Vec3b>(uv)[1];
            p.b = image.at<cv::Vec3b>(uv)[2];
...
int main(int argc, char** argv)
{
	...
	message_filters::Subscriber<PointCloud2> cloud_sub(nh, "/velodyne_points", 10);
	message_filters::Subscriber<Image> image_sub(nh, "/camera1/image_raw", 10);
	message_filters::Subscriber<CameraInfo> cinfo_sub(nh, "/camera1/camera_info", 10);
